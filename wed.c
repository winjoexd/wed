...
int editorRowCxToRx(erow *row, int cx)
{
	...
	{
		if (row->chars[j] == '\t')
			rx += (WED_TAB_STOP - 1) - (rx % WED_TAB_STOP);
		rx++;
	}
	return rx;
}
...
int editorRowRxToCx(erow *row, int rx)
{
	...
	{
		if (row->chars[cx] == '\t')
			cur_rx += (WED_TAB_STOP - 1) - (cur_rx % WED_TAB_STOP);
		cur_rx++;
		if (cur_rx > rx)
			return cx;
	}
...
void editorUpdateRow(erow *row)
{
	...
	free(row->render);
	row->render = malloc(row->size + tabs * (WED_TAB_STOP - 1) + 1);
...
void editorDrawRows(struct abuf *ab)
{
	...
	for (y = 0; y < E.screenrows; y++)
	{
		...
		if (filerow >= E.numrows)
		{
			...
			{
				welcomelen = snprintf(welcome, sizeof(welcome), "WEDitor -- version %s", WED_VERSION);
